...
def findChestStatus(tchest):
    jlist = Journal.GetJournalEntry(time.time()-30000) 
    #not gonna lie, idk if i trust this time.time() function,
    #why do i have to subtract 30000 seconds to get 5 secs ago...
...
def sendgump(facet,location,emptymap,closest,diff):
    ...
    others = [str(facet),str(location),str(closest[0]) + " " + str(diff)]
    ...
    Gumps.AddButton(gd, 5, 5+(i+len(scripts))*30, 11401, 11412, 1, 1, 0)
    Gumps.AddButton(gd, 30, 5+(i+len(scripts))*30, 11401, 11412, 2, 1, 0)
    #Send Gump#
...
#    Gumps.CloseGump(987654)
    ...
    if gd.buttonid == 2:
        ...
        print(closest[0] + "   " + closest[1] + "   " + closest[2])
