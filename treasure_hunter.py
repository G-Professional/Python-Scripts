...
def findChestStatus(tchest):
    if API.InJournal("That appears to be trapped") == True:
        return 6
...
while not gump.IsDisposed: #Stop the script if the gump is closed
    ...
    #this is a timer for certain events
    n += 1
    if n > 50:
        n = 1

    #state 0 script just started
    if state == 0:
        API.SysMsg("Treasure Hunter started.", 33)
        if tchest and tchest.Serial != lastserial:  #this will detect chest state if the script was restarted
            ...
            API.Pause(1)
            state = findChestStatus(tchest)
        else:
            state = 1

    #state = 1
    if state == 1: #Run this only once per map (the list is BIG)
        if location == None and map == None and tchest == None: 
            #No map open, no coords found, no chest found: 
        ...
        if map != None:
            API.SysMsg("Map found, calculating routes.", 33)
            try:
                diff[1] = 99999
                diff[2] = 99999
                diff[3] = 99999
    ...
    # STATE 2A Map opened, Coords calculated, now waiting to find book in CRL
    if state == 2:
        if not book:
            if n == 10:
        ...
        # STATE 2B Map opened, Coords calculated, book nearby
        if book:
    ...
    if state == 3:
        if currentFacet(facet)[1] == currentFacet(API.GetMap())[1]:
            try:
                API.TrackingArrow(location[0], location[1]) 
        ...
        if currentFacet(facet)[1] != currentFacet(API.GetMap())[1]:
            state = 2 #Reset if we leave the correct facet (go to CRL for instance)
            API.RemoveMapMarker('Treasure')
            API.RemoveMarkedTile(location[0], location[1], facettoint(facet))
        if abs(API.Player.X - location[0]) <2 and abs(API.Player.Y - location[1]) <2:
            API.RemoveMapMarker('Treasure')
            API.RemoveMarkedTile(location[0], location[1], facettoint(facet))
            state = 4
            
    #STATE 4 Digging up chest
    if state == 4:
        tchest = findTChest()
        if tchest:
            lastserial = tchest.Serial
            if tchest.Z == tilez:
                API.Pause(1)
                API.SysMsg("Chest Dug up, state = 1", 33)
                state = 5
        if tchest == None:
            ...
            API.WaitForTarget("any",1)
            API.Target(map)
            digging = True #would be better to check for chat entry?
            cd = time.time()
            while digging == True:
                tchest = findTChest()
                if enemyrange(2) or (API.Player.X + API.Player.Y) != position1 or time.time() >= cd + 20:
    ...
    if state == 5:
        ...
        if findTChest():
            ...
            if API.InJournal('The lock quickly yields to your skill.') == True or API.InJournal('That appears to be trapped') == True:
                state = 6
                API.SysMsg("lock picked, state = " + str(state))
            if API.InJournal('This does not appear to be locked.') == True:
                #chest was already opened. could be trapped.
                state = 6
        API.Pause(1)
    ...
    #STATE 6 Looting and reset
    if tchest != lastserial and state == 7:
